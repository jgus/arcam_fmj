...
def test_get_display_info_type(source, byte_val, expected):
    ...
    state._state[DISPLAY_INFO_TYPE.cc] = bytes([byte_val])
    assert state.get(DISPLAY_INFO_TYPE) is expected
...
async def test_set_display_info_type(info_type, byte_val):
    client = MagicMock(spec=Client)
    state = State(client, 1)
    await state.set(DISPLAY_INFO_TYPE, info_type)
    client.request.assert_called_with(
        1, DISPLAY_INFO_TYPE.cc, bytes([byte_val]), 0
    )
